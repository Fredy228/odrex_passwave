...
export class AddPrilegeGroup1728741370149 implements MigrationInterface {
    name = 'AddPrilegeGroup1728741370149'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`privilege\` DROP FOREIGN KEY \`FK_cde798ce5be8f26d974f3cc97ac\``);
        await queryRunner.query(`CREATE TABLE \`privilege_group\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(250) NOT NULL, \`userId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`ALTER TABLE \`privilege\` DROP COLUMN \`role\``);
        await queryRunner.query(`ALTER TABLE \`privilege\` DROP COLUMN \`userId\``);
        await queryRunner.query(`ALTER TABLE \`privilege\` ADD \`access\` enum ('read', 'edit') NOT NULL DEFAULT 'read'`);
        await queryRunner.query(`ALTER TABLE \`privilege\` ADD \`groupId\` int NULL`);
        await queryRunner.query(`ALTER TABLE \`privilege\` ADD \`companyId\` int NULL`);
        await queryRunner.query(`ALTER TABLE \`privilege\` ADD \`hallId\` int NULL`);
        await queryRunner.query(`ALTER TABLE \`privilege\` ADD \`deviceId\` int NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`user_devices\` ADD CONSTRAINT \`FK_e12ac4f8016243ac71fd2e415af\` FOREIGN KEY (\`userId\`) REFERENCES \`user\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`privilege\` DROP COLUMN \`deviceId\``);
        await queryRunner.query(`ALTER TABLE \`privilege\` DROP COLUMN \`hallId\``);
        await queryRunner.query(`ALTER TABLE \`privilege\` DROP COLUMN \`companyId\``);
        await queryRunner.query(`ALTER TABLE \`privilege\` DROP COLUMN \`groupId\``);
        await queryRunner.query(`ALTER TABLE \`privilege\` DROP COLUMN \`access\``);
        await queryRunner.query(`ALTER TABLE \`privilege\` ADD \`userId\` int NULL DEFAULT 'NULL'`);
        await queryRunner.query(`ALTER TABLE \`privilege\` ADD \`role\` enum ('read', 'edit') NOT NULL DEFAULT ''read''`);
